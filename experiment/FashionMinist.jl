...
# -----------------------------------------------------------------------
# 3. History_mnist_exp Tracking System
# -----------------------------------------------------------------------
mutable struct History_mnist_exp
    # Low-Frequency (Evaluations)
    iters::Vector{Int}
    batches_seen::Vector{Int}
    ...
    # High-Frequency Diagnostics (TADAM Only)
    hf_iters::Vector{Int}
    hf_deltas::Vector{Float64}
    hf_rejections::Vector{Float64} 
...
function History_mnist_exp()
    History_mnist_exp(Int[], Int[], Float64[], Float32[], Float32[], Float32[], Float32[], 
            Int[], Float64[], Float64[], 0.0, time_ns(), 0, 0)
end

function evaluate_and_log!(hist::History_mnist_exp, iter::Int, current_batches_seen::Int, ps_vec)
    # Stop timer
    ...
    @printf "Iter %4d | Batches: %4d | Time: %5.1fs | Tr Loss: %.4f | Tr Acc: %.2f%% | Te Acc: %.2f%%\n" iter current_batches_seen hist.active_time tr_loss (tr_acc*100) (te_acc*100)
...
function run_tadam!(nlp; max_iter::Int=1500, eval_freq::Int=50, kwargs...)
    ...
    function tadam_callback(nlp, solver, stats)
        ...
        push!(hist.hf_iters, stats.iter)
        push!(hist.hf_deltas, solver.Δ)
        rej_rate = (hist.n_accepted + hist.n_rejected) == 0 ? 0.0 : hist.n_rejected / (hist.n_accepted + hist.n_rejected)
        push!(hist.hf_rejections, rej_rate)

        # --- LOW FREQUENCY LOGGING (Evaluation) ---
        if stats.iter % eval_freq == 0 || stats.iter == 0
            evaluate_and_log!(hist, stats.iter, batches_seen[], solver.x)
        end
    ...
    if hist.iters[end] != stats.iter
        evaluate_and_log!(hist, stats.iter, batches_seen[], stats.solution)
    end
...
# -----------------------------------------------------------------------
# 6. Publication-Ready Dashboard (8 Panels)
# -----------------------------------------------------------------------
fig = Figure(size=(1200, 1200)) # Increased height for 4 rows
c_adam  = :gray50
c_tadam = :dodgerblue

# Row 1: Train Loss
ax1 = CairoMakie.Axis(fig[1, 1], xlabel = "Minibatches Seen", ylabel = "Train Loss", yscale = log10, title = "Train Loss vs epochs")
ax2 = CairoMakie.Axis(fig[1, 2], xlabel = "Active Time (s)", ylabel = "Train Loss", yscale = log10, title = "Train Loss vs Time")
...
# Row 2: Train Accuracy
ax3 = CairoMakie.Axis(fig[2, 1], xlabel = "Minibatches Seen", ylabel = "Train Accuracy", title = "Train Acc vs epochs")
ax4 = CairoMakie.Axis(fig[2, 2], xlabel = "Active Time (s)", ylabel = "Train Accuracy", title = "Train Acc vs Time")

lines!(ax3, hist_adam.batches_seen,  hist_adam.train_acc,  color=c_adam,  linewidth=2.5)
lines!(ax3, hist_tadam.batches_seen, hist_tadam.train_acc, color=c_tadam, linewidth=2.5)

lines!(ax4, hist_adam.times,  hist_adam.train_acc,  color=c_adam,  linewidth=2.5)
lines!(ax4, hist_tadam.times, hist_tadam.train_acc, color=c_tadam, linewidth=2.5)

# Row 3: Test Accuracy
ax5 = CairoMakie.Axis(fig[3, 1], xlabel = "Minibatches Seen", ylabel = "Test Accuracy", title = "Test Acc vs epochs")
ax6 = CairoMakie.Axis(fig[3, 2], xlabel = "Active Time (s)", ylabel = "Test Accuracy", title = "Test Acc vs Time")

lines!(ax5, hist_adam.batches_seen,  hist_adam.test_acc,  color=c_adam,  linewidth=2.5)
lines!(ax5, hist_tadam.batches_seen, hist_tadam.test_acc, color=c_tadam, linewidth=2.5)

lines!(ax6, hist_adam.times,  hist_adam.test_acc,  color=c_adam,  linewidth=2.5)
lines!(ax6, hist_tadam.times, hist_tadam.test_acc, color=c_tadam, linewidth=2.5)

# Row 4: High-Frequency Diagnostics (TADAM Only)
ax7 = CairoMakie.Axis(fig[4, 1], xlabel = "Iterations", ylabel = "Trust Region Radius (Δ)", yscale = log10, title = "High-Frequency Trust Region Dynamics")
ax8 = CairoMakie.Axis(fig[4, 2], xlabel = "Iterations", ylabel = "Rejection Rate", title = "Cumulative Rejection Rate")

# Note: We now plot the hf_ arrays which contain 1500 data points instead of 30!
lines!(ax7, hist_tadam.hf_iters, hist_tadam.hf_deltas, color=c_tadam, linewidth=1.5)
lines!(ax8, hist_tadam.hf_iters, hist_tadam.hf_rejections, color=c_tadam, linewidth=2.0)
ylims!(ax8, 0.0, 1.0)
