...
# Cast to Float32 to avoid SubArray{Bool} overhead during broadcasting
train_y = Float32.(onehotbatch(tr_y_raw[1:N_TRAIN], 0:9))
test_x  = add_ch(te_x_raw)[:, :, :, 1:N_TEST]
test_y  = Float32.(onehotbatch(te_y_raw[1:N_TEST], 0:9))
...
# ----------------------------------------------------------------
# 6.  Figure (7 panels, colorblind-safe, PDF + PNG)
# ----------------------------------------------------------------
